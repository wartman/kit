...
class JsonSerializerBuildStep implements BuildStep {
	...
	public function apply(build:Build) {
		var ret = options.returnType ?? build.type.toComplexType();
		var fields = build.hook(Init).getProps();
		var serializer:Array<ObjectField> = [];
		var deserializer:Array<ObjectField> = [];

		for (field in fields) {
			var result = parseField(build, field);
		...
		var constructors = FieldCollection.fromTypeDefinition(switch options.constructorAccessor {
			case null:
				var clsTp = build.type.toTypePath();
				macro class {
		...
		var clsParams = build.type.getClass().params.map(param -> ({
			name: param.name,
		...
		var fromJson = constructors.get('fromJson').orThrow().applyParameters(clsParams);
	...
	function parseField(build:Build, prop:Field):JsonSerializerHook {
		var field = build.fields.get(prop.name).or(() -> prop);
		...
		return switch field.kind {
			case FVar(t, _) | FProp(_, _, t):
				var meta = field.meta.get(':json').unwrap();
				var name = field.name;
				var pos = field.pos;
